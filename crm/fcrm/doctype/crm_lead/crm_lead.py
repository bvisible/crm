...
# //// Neoffice — upstream imports `_add` from frappe.desk.form.assign_to: on
# //// frappe/frappe version-15 `add` is a thin whitelisted wrapper and `_add`
# //// carries the body. Our fork is still at the v15.89.0 base, where `add` IS
# //// the body — same signature, same kwargs — so the bare import raised
# //// ImportError and this DocType controller would not load at all. Drop this
# //// shim the day our frappe fork is brought up to current version-15.
try:
