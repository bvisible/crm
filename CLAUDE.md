...
⚠️ **Ne jamais lancer `yarn build` ou `bench build --app crm` localement sur un serveur Neoffice** (4 GB RAM → OOM-kill garanti). Le build se fait UNIQUEMENT sur GitHub Actions (ubuntu-latest, 16 GB RAM).
...
3. Sur les instances clients, le pipeline d'update fait `git pull` (ramène ton commit + le commit du bot). Quand `bench build --app crm` tourne, il appelle `yarn build` à la racine — **le `package.json` voit les artefacts déjà présents et skip vite** (gate). Plus d'OOM-kill.

### Paths spécifiques

- **Source frontend** : `frontend/`
- **Artefacts vite (commités)** : `crm/public/frontend/`
- **SPA HTML(s) (commités)** : `crm/www/crm.html`
- **Build script root** : `yarn (`cd frontend && yarn build`)`
...
### Edge cases spécifiques à crm

- **socket.js**: import statique de `common_site_config.json` retiré, fallback `window.socketio_port || "9000"` (commit `a274058b`).
- **telemetry.ts**: import statique de `posthog.js` retiré (404 en prod, commit `c23157ba`).
