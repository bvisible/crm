...
| **New shims — our frappe fork is behind version-15** | crm v1.83.0 uses five symbols our frappe fork lacks: `frappe.model.delete_doc.get_linked_docs` / `get_dynamic_linked_docs`, `frappe.desk.form.assign_to._add` and the method `User._reset_password` all exist on `frappe/frappe` version-15 but not at our v15.89.0 base; `frappe.utils.telemetry.is_pulse_enabled` is genuinely v16. Each call site sits behind a `try`/`except` import or a `getattr` fallback, marked, with the condition to drop it. **The day the frappe fork is brought up to current version-15, delete these shims — do not reimplement the functions here.** Files: `crm/api/doc.py`, `crm/fcrm/doctype/crm_deal/crm_deal.py`, `crm/fcrm/doctype/crm_lead/crm_lead.py`, `crm/www/crm.py`, `crm/api/__init__.py`, `crm/telemetry.py`. The frontend has one more: `frontend/src/router.js` skips the persona wizard's `frappe.utils.telemetry.pulse.client.boot_config` call (`PULSE_TELEMETRY_AVAILABLE = false`, added 2026-09-12) — that endpoint is genuinely v16 and answered 417 on every CRM load. Its other caller is frappe-ui's `telemetryPlugin` (`frontend/src/main.js`), so the endpoint itself is answered server-side too: `crm.telemetry.pulse_boot_config`, routed by `override_whitelisted_methods` in `crm/hooks.py`, returns `{"enabled": false}` — no 417, and frappe-ui never loads its external Pulse script. |
...
| `frappe-ui` (git submodule pointer) | **Resolved 2026-09-06 (tracker #233): commit `aecf7240` ("Update FR") moved it BACKWARDS.** `3423aa5b` is upstream `frappe-ui` v0.1.120 of 2025-03-24; the pointer it replaced, `c9a0fc93`, is v0.1.200 of 2025-09-22 — and `frontend/package.json` pins the npm package to `0.1.201`. So the bump was a stale local checkout committed by accident, not a decision. Restored to `c9a0fc93`, which matches the npm pin. No production impact either way: the submodule is only an alias source for local dev (`vite.config.js` adds it when `isDev`), the shipped build uses the npm package. | Take upstream's pointer at the merge — it is now `f730c189`, v1.0.0-beta.29, which belongs with a newer crm. |
